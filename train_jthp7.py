...
def myParseArgs():
    ...
    parser.add_argument('--dataroot', help='path to dataset', default='/data/libohong/Data/SZU-cover-resample-256-QF75-DCT')
    ...
    parser.add_argument('--outf', default='jthp7', help='folder to output images and model checkpoints')
...
class SZUDataset256(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        P = sio.loadmat(img_path)

        data = P["dct_matrix"]
        bpnzAC = P["bpnzAC"]

        rand_ud = np.random.rand(256, 256)
        sample = {'data': data, 'rand_ud': rand_ud, 'label': label, 'bpnzAC': bpnzAC}
        if self.transforms:
...
class ToTensor():
    def __call__(self, sample):
        data, rand_ud, label, bpnzAC = sample['data'], sample['rand_ud'], sample['label'], sample['bpnzAC'],

        data = np.expand_dims(data, axis=0)
        data = data.astype(np.float32)
        rand_ud = rand_ud.astype(np.float32)
        rand_ud = np.expand_dims(rand_ud, axis=0)
        # bpnzAC = np.expand_dims(bpnzAC, axis=0)
        bpnzAC = bpnzAC.astype(np.float32)
        ...
        new_sample = {
            ...
            'bpnzAC': torch.from_numpy(bpnzAC),
        }
...
class ToTensor256():
    def __call__(self, sample):
        dct, nzac, hill_cp, rand_ud, label = sample['dct'], sample['nzac'], sample['hill_cp'], sample['rand_ud'], \
        sample['label']
...
def main():
    ...
    Q = sio.loadmat(r'/data/libohong/Data/quant.mat')
    quant = Q['quant']
    quant = torch.tensor(quant, dtype=torch.float32)
    quant = torch.unsqueeze(quant, 0)
    quant = torch.unsqueeze(quant, 0)

    J = sio.loadmat(r'/data/libohong/project/Auto/jt_hp7.mat')
    jt = J['rho']
    jt = torch.tensor(jt, dtype=torch.float32)
    jt = torch.unsqueeze(jt, 0)
    jt = torch.unsqueeze(jt, 0)


    IDCTBase = torch.zeros([64, 1, 8, 8], dtype=torch.float32)
    ...
    for i in range(0, 8):
        for j in range(0, 8):
            for k in range(0, 8):
                for l in range(0, 8):
                    IDCTBase[i * 8 + j, :, k, l] = w[k] * w[l] / 4.0 * math.cos(
                        PI / 16.0 * k * (2 * i + 1)) * math.cos(PI / 16.0 * l * (2 * j + 1))

    IDCTKernel = nn.Parameter(IDCTBase, requires_grad=False)
    ...
    for epoch in range(opt.niter):
        ...
        for i, sample in enumerate(dataloader, 0):
            ...
            data, rand_ud, label, bpnzAC = sample['data'], sample['rand_ud'], sample['label'], sample['bpnzAC']
            cover, n, label, bpnzAC = data.cuda(), rand_ud.cuda(), label.cuda(), bpnzAC.cuda()

            quant = quant.cuda()
            jt = jt.cuda()
            IDCTKernel = IDCTKernel.cuda()

            coverD = torch.mul(quant, cover)
            coverD = nn.functional.conv2d(input=coverD, weight=IDCTKernel, stride=8) + 128
            coverD = F.pixel_shuffle(coverD, 8)
            ...
            stegoD = torch.mul(quant, stego)
            stegoD = nn.functional.conv2d(input=stegoD, weight=IDCTKernel, stride=8) + 128
            stegoD = F.pixel_shuffle(stegoD, 8)

            C = -(p_plus * torch.log2(p_plus) + p_minus * torch.log2(p_minus) + (
                        1 - p_plus - p_minus + 1e-5) * torch.log2(1 - p_plus - p_minus + 1e-5))

            d_input = torch.zeros(opt.batchSize * 2, 1, 256, 256).cuda()

            d_input[0:opt.batchSize * 2:2, :] = coverD
            d_input[1:opt.batchSize * 2:2, :] = stegoD
            ...
            if min_idx == 0:
                ...
                g_l1 = torch.mean(entropy * reward)
                iteration_xu += 1
            ...
            g_l2 = torch.mean((C.sum(dim=(1, 2, 3)) - torch.mean(bpnzAC) * PAYLOAD) ** 2)

            # errG = -g_l1 + 1e-7 * g_l2
            errG = -g_l1 + 1e-7 * g_l2 + 1e-3 * g_l3
            if epoch > 0:
            ...
            logging.info('Epoch: [%d/%d][%d/%d] Loss_D1: %.4f Loss_D2: %.4f  Loss_D3: %.4f Loss_G: %.4f  C:%.4f' %
                         (epoch, opt.niter - 1, i, len(dataloader), errD1.item(), errD2.item(), errD3.item(),
                          errG.item(), C.sum().item() / opt.batchSize))
        ...
        if (epoch + 1) % 10 == 2:
            torch.save(netG.state_dict(), '%s/spuv1_epoch_%s_%d.pth' % (opt.outf, opt.config, epoch + 1))

        logging.info(
            "xunet for %d iters, YedNet for %d iters, SRNet for %d iters" % (iteration_xu, iteration_yed, iteration_sr))
